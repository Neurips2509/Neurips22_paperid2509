...
#edge_index=to_undirected(torch.transpose(torch.tensor(edge_index,dtype=torch.long),0,1))
...
with open('Caccu_restart_%d_selection_%d.txt'%(args.restart,args.max_selections), 'a') as file_object:
    file_object.write('%.4f'%(maxclique/testloadedclique[sample_idx]))
    file_object.write('\n')

with open('Ctime_restart_%d_selection_%d.txt'%(args.restart,args.max_selections), 'a') as file_object:
