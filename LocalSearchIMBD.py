...
with open('Iaccu_restart_%d_selection_%d.txt'%(args.restart,args.max_selections), 'a') as file_object:
    file_object.write('%.4f'%(maxclique/testloadedclique[sample_idx]))
    file_object.write('\n')

with open('Itime_restart_%d_selection_%d.txt'%(args.restart,args.max_selections), 'a') as file_object:
